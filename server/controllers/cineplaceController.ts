...
export const createCineplace = async (req: Request, res: Response) => {
    try {
        const { name, description, movie, images, state, bestTimeToVisit, travelTips, averageBudget } = req.body;

        if (!name || !description || !movie || !state || !images || !bestTimeToVisit || !travelTips || !averageBudget) {
