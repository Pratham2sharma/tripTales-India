...
const CineplaceManager = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setFormData({ name: '', description: '', movie: '', state: '', bestTimeToVisit: '', averageBudget: '', travelTips: '' });
      setSelectedImages([]);
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-black">Cinema Places Management</h1>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4 text-black">Create New Cinema Place</h2>
      ...
      <div className="bg-white rounded-lg shadow-md">
        <div className="p-6 border-b">
          <h2 className="text-xl font-semibold text-black">
            {viewMode === 'list' ? 'All Cinema Places' : 
             viewMode === 'view' ? 'Cinema Place Details' : 'Edit Cinema Place'}
          </h2>
        </div>
        <div className="p-6">
          {loading ? (
            <div className="text-center py-8 text-black">Loading cinema places...</div>
          ) : error ? (
            ...
            {viewMode === 'list' && (
              <div className="space-y-4">
                {cineplaces.map((cineplace) => (
                  <div key={cineplace._id} className="border rounded-lg p-4 hover:shadow-lg transition-shadow">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-black mb-2">{cineplace.name}</h3>
                        <p className="text-black text-sm mb-1"><strong>Movie:</strong> {cineplace.movie}</p>
                        <p className="text-black text-sm mb-3">{cineplace.description}</p>
                      </div>
            ...
            {viewMode === 'view' && selectedCineplace && (
              <div className="bg-white rounded-lg p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-bold text-black">{selectedCineplace.name}</h3>
                  <button
                    onClick={() => {
                      setViewMode('list');
                      clearSelectedCineplace();
                ...
                {detailLoading ? (
                  ...
                  <div className="space-y-4 text-black">
                    <p><strong>Movie:</strong> {selectedCineplace.movie}</p>
                    <p><strong>State:</strong> {selectedCineplace.state}</p>
                    <p><strong>Description:</strong> {selectedCineplace.description}</p>
                    <p><strong>Best Time to Visit:</strong> {selectedCineplace.bestTimeToVisit}</p>
                    <p><strong>Average Budget:</strong> {selectedCineplace.averageBudget}</p>
                    <p><strong>Travel Tips:</strong> {selectedCineplace.travelTips}</p>
                    {selectedCineplace.images && selectedCineplace.images.length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-2">
                          {selectedCineplace.images.map((image, index) => (
                            <img
                              key={index}
                              src={image}
                              alt={`${selectedCineplace.name} ${index + 1}`}
            ...
            {viewMode === 'edit' && selectedCineplace && (
              <div className="bg-white rounded-lg p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-bold text-black">Edit {selectedCineplace.name}</h3>
                  <button
                    onClick={() => {
                      setViewMode('list');
                      clearSelectedCineplace();
